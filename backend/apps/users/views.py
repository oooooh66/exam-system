...
完整调用链（以 POST /api/auth/register/ 为例）：
  ...
  5. ★ View 层 ← request 到达 RegisterView.post()
                      ...
                      │     └─ create() 内部：BusiUser.objects.create(**data) → INSERT INTO users
...
from apps.users.serializers import (
    ...
    BusiUserProfileSerializer,           # 个人信息序列化器
...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        #   3. BusiUser.objects.create(**validated_data) → INSERT INTO users
        #   4. 返回新创建的 User 实例
...
class ProfileView(APIView):
    ...
    def get(self, request):
        ...
        request.user 是从 JWT Token 中解析出来的 User 对象：
          ...
          → Django 查数据库 → BusiUser.objects.get(id=1) → request.user
        """
        # 把 User 实例传给序列化器，它会自动转换为 JSON
        # 内部：遍历 fields 列表，逐个获取属性值，组装成字典，再 json.dumps
        serializer = BusiUserProfileSerializer(request.user)
    ...
    def put(self, request):
        ...
        serializer = BusiUserProfileSerializer(request.user, data=request.data, partial=True)
...
# ============================================
class BusiUserViewSet(mixins.ListModelMixin,      # 列表查询 GET /api/users/
    ...
    # ==================== 基础配置 ====================
    serializer_class = BusiUserManageSerializer          # 默认使用的序列化器
    permission_classes = [IsAdmin]                    # 只有管理员可以操作

    # queryset: 定义查询的数据范围
    #   过滤掉 is_deleted=True 的（软删除的）用户
    queryset = BusiUser.objects.filter(is_deleted=False)
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        调用链：
          ...
          2. 框架调用 self.get_object() → BusiUser.objects.get(id=5, is_deleted=False)
          3. 序列化 → BusiUserManageSerializer(instance).data
    ...
    def create(self, request, *args, **kwargs):
        ...
        调用链：
          1. 创建序列化器 → BusiUserManageSerializer(data=request.data)
          2. 校验 → serializer.is_valid()
          3. 保存 → serializer.save() → create() → make_password() → BusiUser.objects.create()
        ...
        user = serializer.save()
        return APIResponse.success(
            data=BusiUserManageSerializer(user).data,
            message='用户创建成功',
            code=status.HTTP_201_CREATED,
        )
